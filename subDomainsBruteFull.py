...
def processFile(outDir,site,alphaNum):
	#print("开始处理临时文件")
	datas=[]
	...
	for root,dirs,files in os.walk(outDir):
		for file in files:
			if file.endswith('csv'):
				f=open(outDir+'/'+file,"r",encoding='utf-8-sig')
				for line in f:
					if 'uniqlo.cn' in line:
						datas.append(line)			
	...
	for i in datas:
		line=(i.rstrip('\n')).split(',')
		nameToQuery.append(line[1])
		nameToAnswer.append(line[2])
	...
	df=pandas.DataFrame(datasToSave)
	df.to_excel(outDir+'/'+str(alphaNum)+'_result.xlsx')
	print(nameToQuery,nameToAnswer)

	#print("临时文件处理完毕")


def deleteTempFile(outDir,site):
	for root,dirs,files in os.walk(outDir):
		for file in files:
			if file.endswith('csv'):
...
eg:		python subDomainsBruteFullFR_version.py -d baidu.com -o baidu.com -n 2
			"""
	...
	print("总计花费时间：",totalTime)
	runtime=open('./{}/time.txt'.format(outDir),'w')
	runtime.write('总计花费时间：'+str(totalTime)+' s')
	runtime.close

	print("开始处理临时文件")
	site=domainName
	processFile(outDir,site,alphaNum)
	deleteTempFile(outDir,site)
